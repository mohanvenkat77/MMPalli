...
export default function FoundationLedgerTable() {
  ...
  // Fetch data based on the selected tab and page
  ...
  return (
    <div className="w-full">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-slate-100 pb-6 mb-6 gap-4">
        <h2 className="text-2xl font-bold text-trustBlue-900 tracking-tight">Recent Transactions</h2>
      ...
      {isLoading ? (
        <div className="flex flex-col items-center justify-center h-64 text-trustBlue-400">
          ...
          <p className="font-medium text-slate-500">Loading ledger data...</p>
        </div>
      ) : data?.data?.length === 0 ? (
